...
rules:
  ...
  - {id: IMP_011, lvl: S, sev: L, cat: IMP, lang: "*", state: P, met: {type: naming_semantic_check, expr: {name_matches_generic_placeholder_pattern: "(foo|bar|baz|temp|data|obj|val|item|stuff)"}}, det: [{tool: aih, rule: "identifier_token_matching_against_common_placeholder_names"}], act: RCM, rat: "使用无意义占位符命名 -> 代码意图模糊 降低可读性 ; #命名质量 #专业表达"}
  - {id: IMP_012, lvl: M, sev: M, cat: IMP, lang: "*", state: P, met: {type: call_chain_check, expr: {method_call_chain_depth: ">4", on_multiple_object_types: true}}, det: [{tool: aih, rule: "ast_method_call_chain_depth_and_object_type_variance_analysis"}], act: RCM, rat: "方法调用链过长(火车残骸) -> 违反迪米特法则 耦合性高 难以mock和测试 ; #代码结构 #松耦合"}
