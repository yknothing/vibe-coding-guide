...
`--profile` 支持 `python`、`javascript`、`typescript` 和 `all`，默认 `all`。Doctor 要求
Python 3.11+ 和完整核心规则，并为每个所选 profile 运行自动清理的 clean/canary 双探针：
clean 必须 pass，canary 必须由全部适用 detector 提供证据并同时 block `IMP_004`/`IMP_007`。
TypeScript probe 会优先放到现有 `.ts/.tsx` 源文件目录，以适配常见 scoped config。
Doctor 自身不安装、不下载依赖，但 detector 进程和项目配置没有网络沙箱；不能宣称 network-free。
JavaScript 双探针覆盖 `.js/.jsx/.mjs/.cjs`，TypeScript 覆盖 `.ts/.tsx`。monorepo 必须以
各 package 为 `--root` 分别运行；一个 `probe_directory` 不能代表所有 package。
自动目录不匹配 scoped config 时，使用 `--probe-dir src` 等项目内目录显式覆盖；目录不存在或
越出 project root 会 fail closed。
...
| `Node.js` | JavaScript runtime | 运行 ESLint，并通过 `VCG_NODE_BIN` 固定。 | 使用组织批准的 OS package manager/工具链，然后运行 `node --version` |
| `eslint` | JavaScript and TypeScript linter | JavaScript standalone 路径；使用 `no-magic-numbers`。 | `npm install -g eslint` |
| `typescript-eslint` | Project-local TypeScript tooling | 为 `.ts`/`.tsx` 提供 parser 和 flat config。 | `npm install --save-dev eslint @eslint/js typescript typescript-eslint` |
...
若只选择 `--profile javascript`，ESLint 缺失时相应命令可以是 `npm install -g eslint`。

安全边界：这些命令只供人工确认后执行；adapter 和安装器不得静默执行。只使用 PyPI/npm、
组织批准的内部镜像或 pinned/approved toolchain；Do not use `curl | sh` installers；
Windows、no-global-npm 或受控环境应使用等价安装方式。core 只对 ESLint 优先使用
`<project-root>/node_modules/.bin/eslint`，再回退到 hook 的 `PATH`；Ruff/lizard 不从项目
`node_modules/.bin` 执行。TypeScript 还必须提供匹配 `.ts`/`.tsx` 的
`eslint.config.*`；`profiles.typescript.setup` 提供安装、配置、验证和安全提示。
ESLint 配置是可执行代码；doctor 和 strict scan 只能在配置已评审的可信仓库中运行。
安装后必须针对准备启用的 profile 重跑 doctor，例如
`python3 hooks/post_tool_use_quality_gate.py --doctor --profile all --require-tools`，并确认
`strict_ready: true` 以及对应 `profiles.<name>.status: pass`。
...
| `selected_profiles` | 本次实际验证的 profile；默认是全部三个。 |
| `requested_probe_directory` | `--probe-dir` 解析后的绝对目录；未指定时为 `null`。 |
| `profiles` | 每个所选 profile 的双探针状态、detector 证据、tool error、setup 和修复建议。 |
...
| `adapter_launch` | `ready`、通用 CLI argv、Claude hook argv、环境变量、profile scope 和 Node；失败 doctor 的 launch argv 为 `null`。 |
| `checks` | runtime、项目根、规则加载、detector inventory 和 profile smoke 检查项。 |
| `next_steps` | 当前状态下的下一步动作。 |

`profiles.<name>` 的稳定字段包括：`status` (`pass|fail`)、nullable `reason`、
`required_detectors`、`expected_canary_rule_ids`、`probe_directory`、`probes`、`tool_errors`、nullable `setup`
和 nullable `remediation`。已运行时，`probes.clean` 与 `probes.canary` 都包含
`expected_decision`、`expected_rule_ids`、`decision`、`rule_ids`、`detector_evidence`、
`evidence_by_file`、`detectors`、`scanned_files`、`skipped_files` 和 `tool_errors`；
`evidence_by_file` 要求每个 canary 扩展名文件分别具备两条 rule 和全部适用 detector 证据。
前置条件失败时 `probes` 为空且 `probe_directory` 为 `null`。
Native adapter 应在 `adapter_launch.ready: true` 时，给 `generic_cli_argv` 追加 `--files`；
Claude Code 使用 `claude_hook_argv`。两者结合 `adapter_launch.environment`，不要从 GUI
进程的默认 `PATH` 重新猜测工具。`adapter_launch.project_root/rules_dir` 和 argv
固定 doctor 已验收的项目根目录与规则目录；`VCG_NODE_BIN` 固定 ESLint 的 Node runtime；argv 中的
`--scan-profile` 会在 detector 执行前拒绝未验收语言。工具移动或升级后必须重跑 doctor。
无法解析或形成循环符号链接的 `--probe-dir`/`VCG_*_BIN` 必须以结构化失败返回，不得 traceback 或回退到其他路径。
...
| `policy` | 本次 effective complexity threshold、YAML baseline、最终来源和 `scan_profile`。 |
